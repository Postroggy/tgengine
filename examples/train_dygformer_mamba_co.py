"""DyGFormer-Mamba with CUDA co-occurrence kernel on 3090.

Architecture: DyGFormer (patch encoding + co-occurrence channel) with backbone
swapped TransformerBlock -> MambaBlock (3 layers). Co-occurrence computed via
cuda_co_occurrence_freq (our CUDA kernel) instead of in-model Python.

Runs 1 epoch on uci + wikipedia, both on RTX 3090 (CUDA_VISIBLE_DEVICES=1/2/3).
Reports per-epoch wall time + peak GPU mem.
"""
import argparse
import time
...
from tgengine.core import kernels as _kernels
from tgengine.core.kernels import cuda_co_occurrence_freq
from tgengine.models.dygformer import DyGFormer, _CoOccurrenceEncoder, PADDING_ID
...
class _CudaCoOccurrenceEncoder(_CoOccurrenceEncoder):
    """Co-occurrence encoder using our cuda_co_occurrence_freq kernel."""

    def forward(self, a_ids: Tensor, b_ids: Tensor):
        if _kernels.HAS_CUDA_EXT and a_ids.is_cuda and a_ids.dtype == torch.int32:
            a_f32 = a_ids.to(torch.int32)
            b_f32 = b_ids.to(torch.int32)
            a_freq, b_freq = cuda_co_occurrence_freq(a_f32, b_f32)  # (B,K,2)
            a_feat = self.proj(a_freq).sum(dim=2)
            b_feat = self.proj(b_freq).sum(dim=2)
            return a_feat, b_feat
        return super().forward(a_ids, b_ids)
...
class _SelectiveSSM(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _HAS_FAST and x.is_cuda:
            ...
            y = _ssm_fn(u, delta_no_bias, A, B_t, C_t, D=self.D,
                        delta_bias=self.dt_proj.bias.float(), delta_softplus=True)
            return y.transpose(1, 2)
        delta = F.softplus(F.linear(dt_raw, self.dt_proj.weight) + self.dt_proj.bias)
        B_b, L, d = x.shape
        n = self.d_state
        h = torch.zeros(B_b, d, n, device=x.device, dtype=x.dtype)
        ys = []
        for t in range(L):
            dt = delta[:, t]
            A_bar = torch.exp(dt.unsqueeze(-1) * A.unsqueeze(0))
            h = A_bar * h + dt.unsqueeze(-1) * (x[:, t].unsqueeze(-1) * B_s[:, t].unsqueeze(1))
            ys.append((h * C_s[:, t].unsqueeze(1)).sum(-1) + self.D * x[:, t])
...
class DyGFormerMambaCo(DyGFormer):
    """DyGFormer + Mamba backbone + CUDA co-occurrence kernel."""
    ...
    def __init__(self, n_layers: int = 3, **kwargs):
        ...
        self.co_enc = _CudaCoOccurrenceEncoder(self.d_channel, num_nodes=self.co_enc.num_nodes)


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--dataset", default="uci", choices=["uci", "wikipedia"])
    ...
    seed_everything(0)
    _kernels._ensure_cuda_ext()
    ds = load_dataset(args.dataset, args.data_root)
    print(f"[{args.dataset}] {ds.num_edges:,} edges  {ds.num_nodes:,} nodes  d_edge={ds.edge_feat_dim}")
    ...
    print(f"params={sum(p.numel() for p in model.parameters()):,}  "
          f"backbone=Mamba×{args.n_layers}  K={args.K}  co_occurrence=CUDA({_kernels.HAS_CUDA_EXT})  "
          f"ssm_fast={_HAS_FAST}")
    print(f"GPU: {torch.cuda.get_device_name(0)}")

    neg = RandomNegative(ds.num_nodes)
    cfg = TrainConfig(epochs=args.epochs + args.warmup, lr=1e-4, device=args.device,
    ...
    res = eng.train()
    dt = time.time() - t0
    print(f"\n=== {args.dataset} | {torch.cuda.get_device_name(0)} | Mamba×{args.n_layers} K={args.K} co=CUDA ===")
    print(f"total {dt:.1f}s  avg per-epoch {dt/(args.epochs+args.warmup):.2f}s")
    print(f"result: {res}")
